...
patients <- data_com$patients %>%
    ...
    #' covid consult for covid patients, and 31 August 2020 for non-covid patients
...
# .-----
# ----------------- Plots ---------------------------
com_plot <- ggplot(prev_5, aes(x = age_g, y = prevalence, group = cohort, color = cohort)) +
...
acute_covid <- data_com$acute_covid %>%
    ...
    mutate(across(Depression:Parkinsonism, ~ replace_na (.x, 0)))
...
data = pat_com)
{
  ...
  as.data.frame(confint(fit)) %>%
    add_column(log_odds = coef(fit)) %>%
    round(., 2) %>%
    mutate(sample_size = nrow(data))

}

com_odds_ratios <- list(rep(NA, 12))
...
res <- lapply(diseases, odds_contact)

for(i in 1:length(diseases)){
  com_odds_ratios[[i]] <- odds_contact(diseases[i])
}


# save odds ratios
save(com_odds_ratios, com_plot,
     file = here::here("clean_data", "Odds_ratios_comorbidities.RData"))
