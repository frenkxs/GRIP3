...
acute_covid <- data$acute_covid %>%
    mutate(
      start_episode = parse_date(start_episode,'%d%b%Y'),
      end_episode = parse_date(end_episode,'%d%b%Y'),
      ipciendd =    parse_date(ipciendd,'%d%b%Y'),
      ipcistd =     parse_date(ipcistd,'%d%b%Y'),
      patbird =     parse_date(patbird,'%d%b%Y'),
      patdead =     parse_date(patdead,'%d%b%Y'),
...
consults <- data$consultations %>%
  ...
  mutate(
    contactd = parse_date(contactd,'%d%b%Y'),
    consult_week = floor_date(contactd,
                                   unit = "weeks",
                                   week_start = getOption("lubridate.week.start", 1))) %>%
  drop_na(practice)
